...
structure is divided into three top-level directories: `app`, `config`, and `types`.

1. The `app` directory is the core of the Next.js [App Router](https://nextjs.org/docs/app),
   containing routes, components, hooks, and styles.
1. The `config` directory holds configuration files that define global settings and constants.
1. The `types` directory contains TypeScript type definitions and interfaces.
...
Root-level files in the Call of Luci project primarily consist of configuration and metadata files
that define the environment, coding standards, dependencies, and build processes.
